...
export default function Dashboard() {
    ...
    const fetchData = async () => {
      try {
        const [annRes, prayRes] = await Promise.all([
          ...
          api.get('/prayers?limit=5&sortBy=createdAt&sortOrder=desc'),
        ]);
  ...
  return (
    <AppShell>
      {/* Greeting banner */}
      <div className="rounded-xl text-white p-5 mb-6" style={{ background: `linear-gradient(135deg, ${primary}, #6366f1)` }}>
        <h2 className="text-xl font-semibold">
          Good day, {user?.firstName || 'Friend'}
        </h2>
        <p className="text-sm opacity-80 mt-0.5">Here's what's happening in your church today.</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border p-5">
        <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Latest Announcement</h3>
        {loading ? (
          <p className="text-gray-400 text-sm">Loading…</p>
        ) : recentAnnouncement ? (
          <div>
            <h4 className="font-semibold text-gray-800">{recentAnnouncement.title}</h4>
            <p className="text-sm text-gray-600 mt-1 line-clamp-2">{recentAnnouncement.content}</p>
            <a href="/announcements" className="text-sm mt-2 inline-block" style={{ color: primary }}>
