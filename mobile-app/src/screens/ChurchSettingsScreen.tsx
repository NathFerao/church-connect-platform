...
export default function ChurchSettingsScreen({ navigation }: Props) {
  ...
  const handleSave = async () => {
    if (!form.name.trim()) {
      Alert.alert('Error', 'Church name is required');
      return;
    }
    // Basic hex validation for colours
    const hexRegex = /^#[0-9A-Fa-f]{6}$/;
    if (!hexRegex.test(form.primaryColor) || !hexRegex.test(form.secondaryColor)) {
      Alert.alert('Error', 'Colours must be valid hex codes (e.g. #4F46E5)');
      return;
    }
    ...
    try {
      ...
      // Update the church in auth store so theme changes apply immediately
      const updatedChurchObj = {
        id: updatedChurch.id ?? church?.id ?? '',
        name: updatedChurch.name,
        logoUrl: updatedChurch.logoUrl ?? church?.logoUrl ?? null,
        primaryColor: updatedChurch.primaryColor,
        secondaryColor: updatedChurch.secondaryColor,
      };

      if (user && token) {
        await setAuth(user, updatedChurchObj, token);
      }
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: c.background }]} edges={['bottom']}>
      ...
      >
        ...
        <Surface style={[styles.section, { backgroundColor: c.surface }]} elevation={1}>
          ...
          <TextInput label="Church Name *" value={form.name} onChangeText={set('name')}
          ...
          <View style={styles.row}>
            <TextInput label="Email" value={form.email} onChangeText={set('email')}
              mode="outlined" style={[styles.input, styles.flex]} keyboardType="email-address"
              autoCapitalize="none" />
          ...
          <TextInput label="Address" value={form.address} onChangeText={set('address')}
            mode="outlined" style={styles.input} />

          <View style={styles.row}>
            <TextInput label="City" value={form.city} onChangeText={set('city')}
              mode="outlined" style={[styles.input, styles.flex]} />
            <TextInput label="State" value={form.state} onChangeText={set('state')}
              mode="outlined" style={[styles.input, styles.flex]} />
          </View>

          <View style={styles.row}>
            <TextInput label="Country" value={form.country} onChangeText={set('country')}
              mode="outlined" style={[styles.input, styles.flex]} />
            <TextInput label="ZIP Code" value={form.zipCode} onChangeText={set('zipCode')}
              mode="outlined" style={[styles.input, styles.flex]} />
          </View>
        </Surface>
        ...
        <View style={styles.actions}>
          <Button mode="outlined" onPress={() => navigation.goBack()}
            style={styles.actionBtn} disabled={saving}>
            Cancel
          </Button>
