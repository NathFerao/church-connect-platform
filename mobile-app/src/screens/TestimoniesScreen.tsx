...
import { Card, Text, Chip, FAB, Portal, Modal, TextInput, Button, SegmentedButtons } from 'react-native-paper';
...
const CATEGORY_COLORS: Record<Category, string> = {
  SALVATION: '#8B5CF6', HEALING: '#10B981', PROVISION: '#F59E0B',
  RELATIONSHIPS: '#EC4899', OTHER: '#6B7280',
};

const CATEGORY_OPTIONS = [
  { value: 'SALVATION', label: 'Salvation' }, { value: 'HEALING', label: 'Healing' },
  { value: 'PROVISION', label: 'Provision' }, { value: 'RELATIONSHIPS', label: 'Relationships' },
...
export default function TestimoniesScreen() {
  ...
  const onRefresh = async () => { setRefreshing(true); await fetchTestimonies(); setRefreshing(false); };
  const openModal = () => { setTitle(''); setContent(''); setCategory('OTHER'); setModalVisible(true); };

  const handleSubmit = async () => {
    if (!title.trim() || !content.trim()) return;
    setSubmitting(true);
    try {
      await api.post('/testimonies', { title: title.trim(), content: content.trim(), category });
      setModalVisible(false);
      await fetchTestimonies();
    } catch { /* keep modal open */ } finally { setSubmitting(false); }
  };
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: c.background }]} edges={['top']}>
      <FlatList
        ...
        ListHeaderComponent={
          <Text variant="headlineSmall" style={[styles.heading, { color: c.text }]}>Testimonies</Text>
        }
        renderItem={({ item }) => (
        ...
        ListEmptyComponent={!loading ? (
          <Text style={[styles.empty, { color: c.textSecondary }]}>No testimonies yet. Be the first to share!</Text>
        ) : null}
      />
        ...
        <Modal visible={modalVisible} onDismiss={() => setModalVisible(false)}
          ...
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
            <ScrollView showsVerticalScrollIndicator={false}>
              <Text variant="titleLarge" style={[styles.modalTitle, { color: c.text }]}>Share Your Testimony</Text>
              <TextInput label="Title" value={title} onChangeText={setTitle} style={styles.input} mode="outlined" />
              <TextInput label="Your testimony" value={content} onChangeText={setContent}
                style={styles.input} mode="outlined" multiline numberOfLines={6} />
              <Text variant="labelMedium" style={[styles.categoryLabel, { color: c.textSecondary }]}>Category</Text>
              ...
              <View style={styles.modalActions}>
                ...
                <Button mode="contained" onPress={handleSubmit} style={styles.modalBtn}
                  loading={submitting} disabled={submitting || !title.trim() || !content.trim()}>Share</Button>
              </View>
            </ScrollView>
